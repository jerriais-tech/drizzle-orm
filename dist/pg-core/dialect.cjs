...
function shortAlias(name) {
  return (0, import_crypto.createHash)("shake256", { outputLength: 4 }).update(name).digest("hex");
}
class PgDialect {
  ...
  buildRelationalQueryWithoutPK({
    ...
    if (nestedQueryRelation) {
      let field = import_sql2.sql`json_build_array(${import_sql2.sql.join(
        selection.map(
          ({ field: field2, tsKey, isJson }) => isJson ? import_sql2.sql`${import_sql2.sql.identifier(`${tableAlias}_${shortAlias(tsKey)}`)}.${import_sql2.sql.identifier("data")}` : (0, import_entity.is)(field2, import_sql2.SQL.Aliased) ? field2.sql : field2
        ),
        import_sql2.sql`, `
      )})`;
